...
function updatePage(){
  ...
  document.querySelectorAll('#nav-links a').forEach(link=>link.classList.toggle('active',link.dataset.section===active||(active==='identity'&&link.dataset.section==='about')));
...
function drawMesh(time=0){
  ...
  meshEdges.forEach(edge=>{
    ...
    meshContext.strokeStyle=edge.hot?`rgba(47,111,175,${.14*pulse*(1-distance/200)})`:`rgba(180,190,204,${.1*pulse*(1-distance/200)})`;
  ...
  meshNodes.forEach((node,index)=>{
    ...
    meshContext.beginPath();meshContext.arc(positions[index].x,positions[index].y,node.hot?1.8:.7,0,Math.PI*2);meshContext.fillStyle=node.hot?'rgba(47,111,175,.72)':'rgba(205,212,222,.32)';meshContext.fill();
  });
...
function drawBrainGalaxy(time=0){
  ...
  galaxyPoints.forEach((point,index)=>{const breathe=Math.sin(time*.001+point.phase)*3;galaxyContext.beginPath();galaxyContext.arc(point.x+breathe,point.y+breathe*.4,point.r,0,Math.PI*2);galaxyContext.fillStyle=point.hot?'rgba(47,111,175,.85)':'rgba(210,220,232,.42)';galaxyContext.fill();if(index%23===0){const next=galaxyPoints[(index+7)%galaxyPoints.length];galaxyContext.beginPath();galaxyContext.moveTo(point.x,point.y);galaxyContext.lineTo(next.x,next.y);galaxyContext.strokeStyle='rgba(190,201,216,.08)';galaxyContext.stroke()}});
